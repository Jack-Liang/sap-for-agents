...
| Delivery port (flat invoke) | ✅ `POST /api/invokes/{alias}` — registered APIs callable by external systems with flat JSON in/out (case-insensitive keys, true-typed outputs, row caps); the contract is re-derived from the **live server-side interface on every call**, so signature drift is followed immediately (invoking a signature changed since gateway start returns a clear `409 SIGNATURE_STALE` instead of guessing); `GET /openapi.json` serves a typed service catalog of published entries for client codegen |
...
| `SAP_MODE` | ❌ | `full` | Runtime profile (`runtime`): shrinks the gateway to a pure consumer runtime — only `GET /api/registry`, `POST /api/invokes/{alias}` and public pages/probes stay; everything else under `/api/*` and `/mcp` returns `403 RUNTIME_MODE` (see `/api/version` → `capabilities.mode`). Default `full` = the agent workbench |
| `SAP_REGISTRY_FILE` | ❌ | `./registry.json` | API registry file (v0.12, cross-session memory for agents): every remote-enabled FM successfully written through the gateway is auto-registered as a `draft` entry (`GET /api/registry`); deleting the FM tombstones its entry. Local JSON, survives restarts, independent of SAP; a corrupt file is backed up (`.corrupt-<ts>`) and the gateway starts with an empty registry |
